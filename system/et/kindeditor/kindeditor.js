...
function _formatHtml(html, htmlTags, urlType, wellFormatted, indentChar) {
	...
	//bof 把p标签里面的 div p 去掉 2013-12-16 by dawnc
	var escapeTag = function(){
		var inner = arguments[4] ? arguments[4] : "";
	...
	html = html.replace(/(<(p|p\s[^>]*?)>)\s*?<(div|div\s[^>]*?)>([\s\S]*?)<\/div>\s*?(<\/p>)/ig, escapeTag);
	html = html.replace(/(<(p|p\s[^>]*?)>)\s*?<(p|p\s[^>]*?)>([\s\S]*?)<\/p>\s*?(<\/p>)/ig, escapeTag);
	//eof 把p标签里面的 div p 去掉
...
_extend(KUploadButton, {
	...
	submit : function() {
		...
		iframe.bind('load', function() {
            ...
            //bof 跨域问题处理  edit by Dawn
            if(doc.getElementById('packRemoteJsonData')){
